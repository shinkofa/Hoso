...
- **Docs : CDC/PET → repo de code, savoir → Shinzo (BLOCKING)** — répartition des documents générés par Takumi :
  - **CDC + PET** → dans le repo de code du projet (`docs/CDC.md`, `docs/PET.md`). Au plus près du code, lecture facile, économie de tokens.
  - **Savoir, vision, recherche, décisions, stratégie, architecture, infra, écosystème, audits, briefs, et notes de travail vivantes** → **Shinzo** (repo central du savoir, source unique lisible par tous les outils via Git ; Obsidian en est un miroir, pas une destination distincte).
  - **Rapports de session** → `docs/Sessions/` du repo concerné (trace locale au travail).

  **Why** : le savoir doit être cross-projet, cross-machine, lisible par tous les outils (Shinzo) ; le CDC/PET technique vit au plus près du code qu'il décrit. **Décision Jay 2026-06-27** — révise la décision 2026-05-27 (« tout → Obsidian »), jugée moins efficace à l'usage. Propagation aux autres projets via `/sync-repo`.
